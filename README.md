...
Each hit is highlighted on the page — via a non-invasive overlay that never
rewrites the page's own styles — and listed in the popup.
...
| `scan-helpers.js`         | DOM helper functions shared between the scanner and tests: `elementHidesText`, `elementIsA11yHidden`, `isTransparentBg`, `resolveBackgroundColor`, `luminance`, `colorFor`, `snippet`, `directText`, `highlightElement`, `drawMarker`, `clearMarks`, plus `collectRoots`/`deepQuerySelector` for recursing into open shadow roots and same-origin iframe documents. `resolveBackgroundColor` returns `null` when nothing paints a real background (rather than guessing the browser canvas), so the "text color = background" heuristic can't false-positive on self-themed dark pages. Same dual-export pattern as `detectors.js`. |
| `scan.js`                 | The DOM side. Walks text **and comment** nodes (a `<!-- ... -->` never renders, but an LLM ingesting the page's raw HTML/DOM still sees it) across the top document, every open shadow root, and every same-origin iframe document (via `collectRoots`), runs the detectors, uses `elementHidesText` for the CSS-hidden-text heuristic, draws hit markers (candle + numbered badges + outline box) into a per-Document `.piscan-overlay` layer instead of injecting into or restyling page elements, and stashes a serializable summary on `window.__PIScanResult`.                                                                 |
...
Arabic/Indic scripts, SEO keyword stuffing that inserts invisible characters
between every letter, and any article discussing prompt injection (including
